...
if (!result.valid) {
    console.log('Error while validating graph:', result.error)
    process.exit(1);
}

console.log('Starting graph: ' + graph.properties.name);
...
Object.keys(graph.processes).forEach(function(processName) {
    ...
    processes[processName].component = require('./components/' + processes[processName].component)();
        ...
        processes[processName].outPorts[portName] = {
            send: function(data) {
                if (tgt !== undefined) {
                    ...
                    console.log('Lost this data into the aether:', data)
                }
            }
        }
    });
...
while(payloads.length !== 0) {
    ...
    var onData = processes[payload.process].component.inPorts[payload.port].onData;
    
    if (onData) {
        onData(processes[payload.process], payload.data);
    } else {
        console.error('OnData is missing for port', payload.port, 'in process', payload.process, '.')
    }
}
